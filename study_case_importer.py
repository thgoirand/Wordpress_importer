...
# MAGIC %md
# MAGIC
# MAGIC # Import des Study Cases WordPress (Bronze + Silver)
# MAGIC
...
# MAGIC %md
# MAGIC ## 3. Fonctions de transformation des study cases (bronze -> silver)
...
def _build_study_case_content_raw(acf_main):
    ...
    Les blocs testimonial-cards et product-section sont ignores.
    """
    parts = []
...
def transform_study_case_item(item: Dict, site_id: str, site_config: Dict) -> Dict:
    """
    Transforme un item study case WordPress en format standardise pour la table silver.
    """
    wp_id = item.get('id')
...
def run_study_case_import_pipeline(sites_to_import: List[str] = WP_SITES_TO_IMPORT,
    ...
    for site_id in sites_to_import:
        ...
        # Recupere les study cases WordPress
        ...
        print(f"[{site_label}] {len(transformed_items)} study case(s) importe(s) (bronze + silver)")
...
# Import incremental des study cases
...
# Apercu des derniers study cases (silver)
display(spark.sql(f"""
    SELECT
        site_id,
        id,
