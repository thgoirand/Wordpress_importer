...
# MAGIC %md
# MAGIC
# MAGIC # Formatage AI des Produits WordPress
# MAGIC
...
# MAGIC
# MAGIC **Pre-requis:** Executer `product_importer` avant pour alimenter la table `cegid_website_pages`.
...
def get_items_to_process(source_table: str) -> list:
    """
    Identifie les produits qui necessitent un formatage AI.
    Criteres:
...
print(f"{total_count} produit(s) a traiter")
...
def run_ai_formatting(source_table: str = SOURCE_TABLE,
    ...
    """
    Execute le formatage AI sur tous les produits en attente, par batch.
...
# =============================================================================
# Lancer le formatage AI par batch pour les produits
# =============================================================================
...
# Apercu des derniers produits formates
display(spark.sql(f"""
    SELECT
        id,
        title,
...
# Statistiques de formatage des produits
display(spark.sql(f"""
    SELECT
        site_id,
        COUNT(*) AS total,
