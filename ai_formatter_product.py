...
# System prompt for product formatting
AI_PROMPT = (
    "You are an expert in web content formatting. "
    "Convert this WordPress JSON of a product page into clean, well-structured markdown. "
    "The content comes from ACF flexible blocks (header, product arguments, features). "
    "Use headings (##, ###), lists, and format links properly. "
    "Highlight key features and product arguments. "
...
print(f"{total_count} product(s) to process (gold)")
...
def run_ai_formatting(gold_table: str = GOLD_TABLE_FULL,
    ...
    print(f"Processing {total} product(s) in {nb_batches} batch(es) of {batch_size} max")
    ...
    print(f"\n{'='*60}")
    print(f"Formatting completed: {processed}/{total} product(s) processed")
