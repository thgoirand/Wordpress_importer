...
├── ai_formatter_blog.py               # Formatage IA du contenu blog
...
**Scope** : Articles (`post`), landing pages, produits

Post-traitement qui convertit le JSON brut WordPress en markdown structuré. Le processus :

1. Identifie les contenus non formatés ou récemment modifiés (colonne `date_formatted`), hors study cases.
...
- **Formatage IA blog** : après chaque import de contenu (posts, landing pages, produits)
