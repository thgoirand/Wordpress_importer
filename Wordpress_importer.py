...
# MAGIC %md
# MAGIC ### 3.2 Schéma spécifique pour les produits (ACF)

# COMMAND ----------

# Schéma pour les arguments produit (pictogrammes + stats)
PRODUCT_ARGUMENT_SCHEMA = StructType([
    StructField("pictogram_icon", StringType(), True),
    StructField("content", StringType(), True),
])

# Schéma pour les sections de fonctionnalités produit
...
# Schéma pour le header flexible ACF
PRODUCT_HEADER_SCHEMA = StructType([
    StructField("acf_fc_layout", StringType(), True),
    StructField("layout_settings", StringType(), True),  # JSON stringifié
    StructField("header_content", StringType(), True),
    StructField("header_content_display", StringType(), True),
    StructField("header_add_media", StringType(), True),
    StructField("header_image_desktop", IntegerType(), True),
    StructField("header_form", StringType(), True),  # JSON stringifié
])
...
PRODUCT_SCHEMA = StructType([
    ...
    StructField("header_content_display", StringType(), True),
    StructField("header_image_desktop", IntegerType(), True),
    StructField("header_form_config", StringType(), True),  # JSON stringifié
...
class WordPressConnector:
    ...
    def transform_product_item(self, item: Dict) -> Dict:
        """
        Transforme un item produit WordPress avec ACF en format standardisé pour Databricks.

        Schéma cible: gdp_cdt_dev_04_gld.sandbox_mkt.cegid_products
        Gère les champs ACF spécifiques (vah_flexible_header, vah_flexible_main)
        """
        wp_id = item.get('id')
...
# MAGIC %md
# MAGIC ### 6.2 Fonctions de gestion de table pour les produits
...
def create_product_table_if_not_exists(catalog: str, schema: str, table_name: str):
    ...
    # Vérifie si la table existe
    if not spark.catalog.tableExists(full_table_name):
        print(f"📝 Création de la table produits {full_table_name}...")

        # Crée une DataFrame vide avec le schéma produit
        empty_df = spark.createDataFrame([], PRODUCT_SCHEMA)

        # Écrit en Delta avec partitionnement par site
        empty_df.write \
            .format("delta") \
            .partitionBy("site_id") \
            .option("delta.enableChangeDataFeed", "true") \
        ...
        print(f"✅ Table produits {full_table_name} créée avec succès")
    else:
        print(f"ℹ️ Table produits {full_table_name} existe déjà")


def get_last_imported_product_id(catalog: str, schema: str, table_name: str,
                                  site_id: str) -> Optional[int]:
    """Récupère le dernier ID WordPress importé pour les produits d'un site."""
...
def upsert_products(df: DataFrame, catalog: str, schema: str, table_name: str):
    ...
    Met à jour les produits existants, insère les nouveaux.
    """
    ...
    print(f"✅ Upsert produits terminé dans {full_table_name}")
...
# MAGIC %md
# MAGIC ### 7.2 Pipeline d'import des produits
...
def run_product_import_pipeline(sites_to_import: List[str] = WP_SITES_TO_IMPORT,
                                 incremental: bool = True):
    """
    Exécute le pipeline d'import des produits pour un ou plusieurs sites.
    ...
    catalog = PRODUCT_DATABRICKS_CONFIG["catalog"]
    schema = PRODUCT_DATABRICKS_CONFIG["schema"]
    table_name = PRODUCT_DATABRICKS_CONFIG["table_name"]

    # Crée la table produits si nécessaire
    ...
    for site_id in sites_to_import:
        ...
        # Récupère les produits WordPress via /wp-json/wp/v2/product
        items = connector.fetch_all_content(
            content_type="product",
            endpoint="/product",
            since_id=since_id
        ...
        if not items:
            print(f"ℹ️ Aucun nouveau produit à importer pour {site_label}")
            continue

        # Transforme les items avec la méthode spécifique aux produits
        transformed_items = [
            connector.transform_product_item(item)
            for item in items
        ]

        # Crée le DataFrame avec le schéma produit
        ...
        total_imported += len(transformed_items)
        print(f"📊 [{site_label}] {len(transformed_items)} produit(s) importé(s)")

    print(f"\n{'#'*60}")
    print(f"🎉 Import produits terminé! Total: {total_imported} produits importés")
    print(f"   Sites traités: {', '.join(sites_to_import)}")
    print(f"{'#'*60}")
...
# Import des produits de TOUS les sites configurés
...
# MAGIC %md
# MAGIC ### 9.2 Vérification des produits

# COMMAND ----------

# Affiche un aperçu des produits importés par site
product_table = f"{PRODUCT_DATABRICKS_CONFIG['catalog']}.{PRODUCT_DATABRICKS_CONFIG['schema']}.{PRODUCT_DATABRICKS_CONFIG['table_name']}"
...
# Exemple: Aperçu des produits avec leurs données ACF
display(spark.sql(f"""
    SELECT
        site_id,
        wp_id,
...
# Exemple: Détail des arguments produit pour un produit spécifique
display(spark.sql(f"""
    SELECT
        wp_id,
        title,
